...
export default function VesselApi() {
  ...
  const [currentApiName, setCurrentApiName] = useState(null);
  ...
  // Fetch current API details on component mount
  useEffect(() => {
    const fetchCurrentApi = async () => {
      try {
        const res = await axios.get(`${baseURL}/api/vessel-api/current`);
        const { apiName, apiUrl } = res.data;

        setCurrentApiName(apiName);
        setCurrentApiUrl(apiUrl);
      } catch (error) {
        console.error('Error fetching current API details:', error);
      }
    };
...
const handleNewFieldChange = (apiField, dataType = 'String', targetName = '') => {
 ...
 const removeNewField = (apiField) => {
  ...
  <Typography variant="h6" mt={2}>Current API Details</Typography>
      <Box sx={{ mb: 3 }}>
        {/* Display current API details in TextField components as read-only */}
        <Typography variant="body1" sx={{ mb: 1 }}>
          <strong>API Name:</strong> {currentApiName}
        </Typography>
        <Typography variant="body1" sx={{ mb: 1 }}>
    ...
    {Object.entries(mapping).filter(([, apiField]) => apiField).map(([aisField, apiField]) => (
      <TableRow key={aisField}>
        ...
        <TableCell>
          <Autocomplete
            freeSolo
            options={flattenKeys(sample).filter(
              (option) => option === apiField || !usedApiFields.has(option)
            )}
            value={apiField}
            onChange={(e, newValue) => {

                if (!newValue) {
    // Just ignore clearing selection — do not assign null mappings
    return;
  }
        ...
        <TableCell>
          <IconButton
            onClick={() => handleMappingChange(aisField, '')}
...
<TableBody>
  ...
  // Call the common handler to update unmatched fields
  handleMappingChange(newValue, apiField);
            ...
            renderInput={(params) => (
              <TextField
                ...
                disabled={isNewField} // Disable mapping if added as new field
              />
            )}
          />
        </TableCell>
