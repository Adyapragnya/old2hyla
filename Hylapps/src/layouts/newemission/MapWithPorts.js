...
const createPointIcon = (width, height) => {
  return L.divIcon({
    ...
    html: `<div style="width: ${width}px; height: ${height}px; background-color: green; border-radius: 50%;"></div>`,
    iconSize: [width, height],
  });
};
...
const MapWithPorts = ({selectedPort }) => {
  const map = useMap();
  ...
  const updateIconsForZoom = useCallback(() => {
    ...
    Object.values(markersRef.current).forEach((marker) => {
      const selectedPort = marker.options.portData;
      const isSelected = selectedPort && selectedPort.name === selectedPort.name;

      const newIcon =
        isSelected
          ? createCustomIcon(163, width , height , type) // Highlight selected vessel
          : type === 'point'
          ? createPointIcon(width, height)
          : createCustomIcon(163, width, height, type);
    ...
    if (map) {
      ...
      if (JSON.stringify(vessels) !== JSON.stringify(prevVesselsRef.current)) {
        ...
        if(selectedPort)  {
          ...
          const key = selectedPort.name || `${selectedPort.lat}-${selectedPort.long}`;
          if (!markersRef.current[key]) {
            ...
            const marker = L.marker([selectedPort.lat, selectedPort.long], {
              icon: type === 'point' ? createPointIcon(width, height) : createCustomIcon(163, width, height, type),
              portData: port,
            });
// remove heading
            ...
            const popupContent = 
              `<div class="popup-container">
                <div class="popup-header">
                  <h3 class="popup-title">${port.name || 'No name'} ></h3>
                </div>
  ...
  }, [map, selectedPort, updateIconsForZoom]);
